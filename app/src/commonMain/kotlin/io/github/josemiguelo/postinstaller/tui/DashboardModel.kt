...
import io.github.josemiguelo.postinstaller.core.diff.DiffEngine
import io.github.josemiguelo.postinstaller.core.diff.InstallState
...
class DashboardModel(private val app: AppContext) {
    ...
    private suspend fun preparePlan(requested: List<String>) {
        ...
        if (installs.any { "sudo" in it.command } && !app.runner.capture("sudo -n true").success) {
            state = state.copy(
                mode = Mode.NORMAL,
                message = "sudo needs a password — run 'sudo -v' in a terminal first, or use the CLI: post-installer install",
            )
            return
        }
        pendingPlan = plan
